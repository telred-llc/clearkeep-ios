...
static const CGFloat kDirectRoomBorderWidth = 1.0;
...
- (void)customizeTableViewCellRendering {
    ...
    [self.roomOnlineStatus.layer setCornerRadius:self.roomOnlineStatus.frame.size.width / 2];
    self.roomOnlineStatus.clipsToBounds = YES;
    self.roomOnlineStatus.layer.borderColor = UIColor.whiteColor.CGColor;
    self.roomOnlineStatus.layer.borderWidth = kDirectRoomBorderWidth;

    // Prepare direct room border
    // Commenting out due to no longer using
...
- (void)render:(MXKCellData *)cellData {
    ...
    if (roomCellData) {
        ...
        if (roomCellData.roomSummary.room.isDirect) {
            self.roomOnlineStatus.hidden = NO;
            NSString *directID = roomCellData.roomSummary.room.directUserId;
            if (directID) {
                MXUser *user = [[AppDelegate.theDelegate mxSessions].firstObject userWithUserId:directID];
                BOOL online = (user.presence == MXPresenceOnline) ? YES : NO;
                self.roomOnlineStatus.backgroundColor = (online) ? UIColor.greenColor : UIColor.lightGrayColor;
            } else {
                self.roomOnlineStatus.backgroundColor = UIColor.lightGrayColor;
            }
        } else {
            self.roomOnlineStatus.hidden = YES;
        }
