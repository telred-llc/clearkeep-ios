...
final class KeyBackupSetupSuccessFromRecoveryKeyViewController: UIViewController {
    ...
    class func instantiate(with recoveryKey: String) -> KeyBackupSetupSuccessFromRecoveryKeyViewController {
        let viewController = StoryboardScene.KeyBackupSetupSuccessFromRecoveryKeyVC.initialScene.instantiate()
    ...
    private func setupViews() {
        ...
        self.informationLabel.text = CKLocalization.string(byKey: "key_backup_setup_success_from_recovery_key_info")
        
        self.recoveryKeyTitleLabel.text = CKLocalization.string(byKey: "key_backup_setup_success_from_recovery_key_recovery_key_title")
        self.recoveryKeyLabel.text = self.recoveryKey
        
        self.makeACopyButton.setTitle(CKLocalization.string(byKey: "key_backup_setup_success_from_recovery_key_make_copy_action"), for: .normal)
        self.madeACopyButton.setTitle(CKLocalization.string(byKey: "key_backup_setup_success_from_recovery_key_made_copy_action"), for: .normal)
    ...
    private func bindingTheme() {
        ...
        themeService.rx
            ...
            .bind({ $0.primaryBgColor }, to: self.recoveryKeySectionBackgroundView.rx.backgroundColor, self.madeACopyButtonBackgroundView.rx.backgroundColor)
            .bind({ $0.primaryTextColor }, to: self.keyBackupLogoImageView.rx.tintColor, self.titleLabel.rx.textColor, self.informationLabel.rx.textColor, self.recoveryKeyTitleLabel.rx.textColor, self.recoveryKeyLabel.rx.textColor)
            .bind({ $0.separatorColor }, to: self.separatorView.rx.backgroundColor)
            .disposed(by: disposeBag)
         
        CKColor.applyStyle(onButton: self.makeACopyButton)
        CKColor.applyStyle(onButton: self.madeACopyButton)
    }
